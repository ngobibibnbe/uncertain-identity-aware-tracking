...
def process_forwad_backward(track_with_observation,nbr_visit="", json_save_path="/home/sophie/HMM-Tracking/Bytetrack/videos/"+video_prev_name+"_with_atq_tracking_with_HMM_resut.json"):
    ...
    Returns:
        write a video with ATQ and put the results in the file /home/sophie/HMM-Tracking/Bytetrack/videos/"+video_prev_name+"_with_atq_tracking_with_HMM_resut.json
    """
    import numpy as np 
    def softmax(x):
        #return x/sum(x)
    ...
    video_path=Home_folder+"/videos/"+video_prev_name+".mp4"
    cap = cv2.VideoCapture(video_path)
    width = cap.get(cv2.CAP_PROP_FRAME_WIDTH)  # float
    height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)  # float
    fps = cap.get(cv2.CAP_PROP_FPS)
    save_path= Home_folder+"/videos/"+video_prev_name+"_with_atq"+str(nbr_visit)+".mp4"
